...
/*! typedef elemType (*devArg1XY)(elemType*, elemType*, int);
...
struct templateFunc
{
	typedef elemType (*devArg1XY)(elemType*, elemType*, int, int, int, int);
};
...
{	
	...
	if (blockIdx.x == 0 && blockIdx.y == 0)
	{
		...
		sum = ((typename templateFunc<elemType>::devArg1XY)func)(arrayLocal, coeLocal, stenSet, nxLocal, numStenHoriz, numStenVert);
...
template
__global__ void kernel2DXYnpFun<double>
(
	double*,  				
	double*,					
	double*, 				
...
template
void cuStenCompute2DXYnpFun<double>
(
	cuSten_t<double>*,
	bool
);
...
template
__global__ void kernel2DXYnpFun<float>
(
	float*,  				
	float*,					
	float*, 				
...
template
void cuStenCompute2DXYnpFun<float>
(
	cuSten_t<float>*,
	bool
);

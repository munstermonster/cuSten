...
/*! \file custenCreateDestroy2DYnpFun.cu
...
) 
{
	...
	switch(pt_cuSten->numTiles)
	{
		...
		case 1:
			pt_cuSten->boundaryTop[0] = &dataInput[0];
			pt_cuSten->boundaryBottom[0] = &dataInput[(pt_cuSten->ny - pt_cuSten->numStenBottom) * pt_cuSten->nx]; 
		...
		case 2:
			...
			pt_cuSten->boundaryBottom[1] = &dataInput[(pt_cuSten->ny - pt_cuSten->numStenBottom) * pt_cuSten->nx];
		...
		default:
			...
			pt_cuSten->boundaryBottom[pt_cuSten->numTiles - 1] = &dataInput[(pt_cuSten->ny - pt_cuSten->numStenBottom) * pt_cuSten->nx];
...
template
void cuStenSwap2DYnp<double>(
	cuSten_t<double>*,
	double*
);

template
void cuStenDestroy2DYnp<double>(
	cuSten_t<double>*
);

template
void cuStenCreate2DYnp<float>(
	cuSten_t<float>*,		
	int,				
	int,				
	int,				
...
template
void cuStenSwap2DYnp<float>(
	cuSten_t<float>*,
	float*
);

template
void cuStenDestroy2DYnp<float>(
	cuSten_t<float>*
);
